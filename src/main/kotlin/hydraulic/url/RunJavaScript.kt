    ...
    return Context.newBuilder("js")
        ...
        .build().use { js ->
            ...
            bindings.putMember("urls", ProxyExecutable { arguments: Array<out Value> ->
                require(arguments.size == 1) { "run.js function urls() expects one object" }
                val request = arguments[0]
                require(request.hasMembers() && !request.hasArrayElements()) {
                    "run.js function urls() expects an object"
                }
                val values = request.getMemberKeys().associateWith { name ->
                    val value = request.getMember(name)
                    require(value.isString) { "run.js URL '$name' must be a string" }
                    value.asString()
                }
                ProxyObject.fromMap(resolve(values).mapValues { it.value.toAbsolutePath().normalize().toString() })
            })
